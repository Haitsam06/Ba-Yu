...
import { Bookmark, Star, LayoutGrid, Clock, ChevronRight, Eye, Heart, MessageCircle } from 'lucide-react';
                        ...
                        <div className="flex items-center justify-between mt-auto">
                           <div className="flex items-center gap-3">
                              {(() => {
                                 ...
                                 return (
                                   <Link to={`/profile/${hAuthor?.id || hAuthor?._id}`} className="flex items-center gap-3 group/hauth">
                     ...
                     <article key={note.id} className="group flex flex-col-reverse sm:flex-row items-center sm:items-start justify-between gap-6 sm:gap-8 py-8 border-b border-gray-100 last:border-0 hover:bg-gray-50/50 transition-colors">
                        ...
                        <div className="flex-1 min-w-0 flex flex-col w-full">
                           ...
                           <div className="flex items-center gap-2 mb-3 flex-wrap">
                              <Link to={`/profile/${author?.id || author?._id}`} className="flex items-center gap-2 group/author outline-none">
                                <img src={author?.avatar} alt={author?.name} className="w-[24px] h-[24px] rounded-full object-cover ring-2 ring-transparent group-hover/author:ring-primary/20 transition-all" />
                                <span className="text-[13px] font-medium text-gray-900 group-hover/author:underline">{author?.name}</span>
                              </Link>
                              <span className="text-gray-300 text-[10px] hidden sm:inline-block">•</span>
                              ...
                              <span className="text-[12px] font-medium text-gray-500 bg-gray-50 px-2 py-0.5 rounded-md hidden sm:inline-block">
                                {note.jenjang === 'Kuliah' ? `${note.kelas} Semester ${note.semester || 1}` : `${note.jenjang} Kelas ${note.kelas}`}
                              </span>
                           </div>
                           
                           {/* Title */}
                           <Link to={`/note/${note.id}`} className="block mb-2 outline-none font-['Lexend_Deca']">
                        ...
                        return (
                           <div key={trend.id} className="group relative flex gap-4 items-start">
                              ...
                              <div className="flex flex-col flex-1 min-w-0 pt-1.5">
                                 <Link to={`/profile/${tAuth?.id || tAuth?._id}`} className="flex items-center gap-1.5 mb-2 hover:opacity-80 transition-opacity">
