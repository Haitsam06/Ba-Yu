...
import { Bell, Shield, AlertCircle, CheckCheck, Heart, MessageCircle, UserPlus } from "lucide-react"
import {
  Popover,
  PopoverTrigger,
  PopoverContent,
...
export default function AvatarNotifications() {
  ...
  const getIcon = (type: string) => {
    if (type === 'sertifikasi') return <Shield className="w-4 h-4 text-indigo-500" />
    if (type === 'report') return <AlertCircle className="w-4 h-4 text-rose-500" />
    if (type === 'verifikasi') return <CheckCheck className="w-4 h-4 text-emerald-500" />
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button className="relative inline-flex items-center justify-center rounded-full p-[6px] hover:bg-gray-100 transition-colors focus:outline-none">
          <Bell className="h-[22px] w-[22px] text-gray-700" strokeWidth={1.5} />
          <span
            className={cn(
      ...
      <PopoverContent className="w-80 p-0 rounded-2xl shadow-xl overflow-hidden border border-gray-100" side="bottom" align="end" sideOffset={8}>
        <div className="max-h-[350px] overflow-y-auto scrollbar-hide">
          <div className="flex justify-between items-center px-4 py-3 bg-gray-50/90 backdrop-blur-md sticky top-0 z-10 border-b border-gray-100">
            <div>
              <h2 className="text-sm font-bold font-['Lexend_Deca'] text-gray-900">Notifikasi</h2>
              <p className="text-[11px] text-gray-500 font-['Manrope']">{hasNotifications ? `${unreadCount} belum dibaca` : 'Semua sudah dibaca'}</p>
            </div>
            {hasNotifications && (
              <Button
                variant="ghost"
          ...
          {notifications.length === 0 ? (
            ...
            <ul className="divide-y divide-gray-50">
              {notifications.map((item) => (
                <li
                  key={item._id}
                  onClick={() => handleMarkAsRead(item._id, item.is_read, item.type, item.link)}
                  className={cn(
                    "flex items-start gap-3 p-4 transition-colors cursor-pointer",
                    !item.is_read ? "bg-primary/[0.03] hover:bg-primary/[0.05]" : "hover:bg-gray-50/50"
                  )}
                >
                  <Avatar className={cn("h-9 w-9 mt-0.5 border border-gray-100 shadow-sm",
                    ...
                    <AvatarFallback className="bg-transparent">{getIcon(item.type)}</AvatarFallback>
                  </Avatar>
                  <div className="flex flex-col flex-1 min-w-0">
                    <div className="flex justify-between items-start mb-0.5">
                      <span className={cn("font-semibold text-[13px] font-['Lexend_Deca'] line-clamp-1", !item.is_read ? "text-gray-900" : "text-gray-700")}>
                        {item.title}
                      </span>
